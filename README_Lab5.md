...
-----------------
*Алгоритм бинарного поиска* - это алгоритм поиска в отсортированном массиве путем деления области поиска пополам.

*Объяснение работы кода*: Сравнение с средним элементом, выбор левой или правой половины для продолжения поиска.

*Временная сложность этого алгоритма* — O(log n), потому что на каждом шаге область поиска уменьшается вдвое. Максимальное число шагов: log₂n.

При запуске кода на примере массива [2, 5, 7, 10, 13, 17, 20]. Мы ищем число 17, то консоль выведет индекс 5 (при индексации с 0). Если бы числа не было в списке, то алгоритм вывел бы результат -1.

10)Алгоритм интерполирующего поиска
--------------------------------
*Алгоритм интерполирующего поиска* - это алгоритм улучшенного бинарного поиска, который оценивает позицию искомого элемента на основе значений.

*Объяснение работы кода*: Вычисление вероятной позиции pos = low + ((x - arr[low]) * (high - low)) / (arr[high] - arr[low]), Рекурсивный поиск в вычисленной области.

*Временная сложность этого алгоритма* — O(n) (при неравномерном распределении, потому что алгоритм может деградировать до линейного сканирования, проверяя каждый элемент по одному), но в лучшем случае может быть — O(log log n) (при равномерном распределении, потому что на каждом шаге размер области поиска уменьшается быстрее, чем в бинарном поиске).

При запуске кода на примере массива [7, 18, 23, 2, 12]. Мы ищем число 23 то консоль выведет индекс 2 (при индексации с 0).

11)Алгоритм Фибоначчи поиска
----------------------------
*Алгоритм Фибоначчи поиска* - это алгоритм поиска, использующий числа Фибоначчи для деления массива.

*Объяснение работы кода*: Использует числа Фибоначчи для определения точек разделения и сравнивает элемент с значениями в позициях Фибоначчи.

*Временная сложность этого алгоритма* — O(log n), потому что алгоритм всегда делит массив в золотом сечении (≈1.618), что дает асимптотику O(log n), аналогичную бинарному поиску, но с лучшей константой.
