...
=======
Коэффициент аппроксимации жадного алгоритма для задачи о покрытии множеств составляет H_n \approx \ln n (где n — количество элементов в универсальном множестве).
...
**Итог:**
Жадный алгоритм находит покрытие, размер которого не превышает OPT \cdot H_n. Это считается очень хорошим результатом, так как доказано, что добиться существенно лучшего приближения за полиномиальное время невозможно (если P \neq NP).
