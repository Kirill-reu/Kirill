...
### 2) Алгоритм работы по блокам:
   ...
   - Backpropagation - это алгоритм для эффективного вычисления градиентов функции потерь по всем параметрам нейронной сети. Он работает в два этапа:
       1) Forward Pass (Прямой проход): Вычисление выхода сети и потери
       2) Backward Pass (Обратный проход): Вычисление градиентов от выхода ко входу
   - Правило цепочки - это фундаментальное правило дифференциального исчисления: если z = f(y) и y = g(x), то: dz/dx = dz/dy * dy/dx
     В контексте нейронных сетей это означает: Градиент потерь относительно весов = градиент потерь относительно выходов слоя × градиент выходов слоя относительно весов
   - Итог: Backpropagation - это систематическое применение правила цепочки для вычисления того, как каждый вес в сети влияет на конечную ошибку, начиная с выходного слоя и двигаясь назад к входному.
